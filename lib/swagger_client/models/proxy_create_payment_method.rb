...
module SwaggerClient
  ...
  class ProxyCreatePaymentMethod
    #  The ID of the customer account associated with this payment method. This field is not required for the subscribe call, but it is required for all other calls including the create call. 
    ...
    #  Line 1 for the ACH address. **Character limit:** **Values:** an address 
    attr_accessor :ach_address1

    #  Line 2 for the ACH address. **Character limit:** **Values:** an address 
    ...
    #  The sort code or number that identifies the bank. This is also known as the sort code. **Character limit**: 18 **Values**:  string of 18 characters or fewer 
    ...
    def list_invalid_properties
      ...
      if !@num_consecutive_failures.nil? && @num_consecutive_failures > 100
        invalid_properties.push("invalid value for 'num_consecutive_failures', must be smaller than or equal to 100.")
      end

      if !@num_consecutive_failures.nil? && @num_consecutive_failures < 0
        invalid_properties.push("invalid value for 'num_consecutive_failures', must be greater than or equal to 0.")
      end
    ...
    def valid?
      ...
      return false if !@num_consecutive_failures.nil? && @num_consecutive_failures > 100
      return false if !@num_consecutive_failures.nil? && @num_consecutive_failures < 0
    ...
    # @param [Object] num_consecutive_failures Value to be assigned
    def num_consecutive_failures=(num_consecutive_failures)

      if !num_consecutive_failures.nil? && num_consecutive_failures > 100
        fail ArgumentError, "invalid value for 'num_consecutive_failures', must be smaller than or equal to 100."
      end

      if !num_consecutive_failures.nil? && num_consecutive_failures < 0
        fail ArgumentError, "invalid value for 'num_consecutive_failures', must be greater than or equal to 0."
      end

      @num_consecutive_failures = num_consecutive_failures
    end
    ...
    def hash
      [account_id, ach_aba_code, ach_account_name, ach_account_type, ach_address1, ach_address2, ach_bank_name, bank_branch_code, bank_check_digit, bank_city, bank_code, bank_name, bank_postal_code, bank_street_name, bank_street_number, bank_transfer_account_name, bank_transfer_account_type, bank_transfer_type, business_identification_code, city, country, credit_card_address1, credit_card_address2, credit_card_city, credit_card_country, credit_card_expiration_month, credit_card_expiration_year, credit_card_holder_name, credit_card_postal_code, credit_card_state, credit_card_type, device_session_id, email, existing_mandate, first_name, gateway_option_data, iban, ip_address, last_name, last_transaction_date_time, last_transaction_status, mandate_creation_date, mandate_id, mandate_received, mandate_update_date, max_consecutive_payment_failures, num_consecutive_failures, payment_retry_window, paypal_baid, paypal_email, paypal_preapproval_key, paypal_type, phone, postal_code, second_token_id, state, street_name, street_number, token_id, total_number_of_error_payments, total_number_of_processed_payments, type, use_default_retry_rule].hash
    end
