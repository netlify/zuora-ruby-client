...
module Zuora
  class EndConditions
    # Specify how the charge is terminated.
    attr_accessor :end_date_condition

    # Specify only when the endDateCondition is 'Specific_End_Date'.
    attr_accessor :specific_end_date

    # Specify only when the endDateCondition is 'Fixed_Period'.
    ...
    def valid?
      end_date_condition_validator = EnumAttributeValidator.new('String', ['Subscription_End', 'Fixed_Period', 'Specific_End_Date'])
      return false unless end_date_condition_validator.valid?(@end_date_condition)
      up_to_periods_type_validator = EnumAttributeValidator.new('String', ['Billing_Periods', 'Days', 'Weeks', 'Months', 'Years'])
      return false unless up_to_periods_type_validator.valid?(@up_to_periods_type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] end_date_condition Object to be assigned
    def end_date_condition=(end_date_condition)
      validator = EnumAttributeValidator.new('String', ['Subscription_End', 'Fixed_Period', 'Specific_End_Date'])
      unless validator.valid?(end_date_condition)
        fail ArgumentError, 'invalid value for "end_date_condition", must be one of #{validator.allowable_values}.'
      end
      @end_date_condition = end_date_condition
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] up_to_periods_type Object to be assigned
    def up_to_periods_type=(up_to_periods_type)
      validator = EnumAttributeValidator.new('String', ['Billing_Periods', 'Days', 'Weeks', 'Months', 'Years'])
      unless validator.valid?(up_to_periods_type)
        fail ArgumentError, 'invalid value for "up_to_periods_type", must be one of #{validator.allowable_values}.'
      end
    ...
    def hash
      [end_date_condition, specific_end_date, up_to_periods, up_to_periods_type].hash
    end
