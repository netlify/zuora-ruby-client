...
module Zuora
  ...
  class LastTerm
    ...
    def list_invalid_properties
      ...
      if !@period.nil? && @period > 100
        invalid_properties.push('invalid value for "period", must be smaller than or equal to 100.')
      end

      if !@period.nil? && @period < 0
        invalid_properties.push('invalid value for "period", must be greater than or equal to 0.')
      end
    ...
    def valid?
      return false if !@period.nil? && @period > 100
      return false if !@period.nil? && @period < 0
      period_type_validator = EnumAttributeValidator.new('String', ['Month', 'Year', 'Day', 'Week'])
      return false unless period_type_validator.valid?(@period_type)
      return false if @term_type.nil?
      term_type_validator = EnumAttributeValidator.new('String', ['TERMED', 'EVERGREEN'])
    ...
    def period=(period)
      if !period.nil? && period > 100
        fail ArgumentError, 'invalid value for "period", must be smaller than or equal to 100.'
      end

      if !period.nil? && period < 0
        fail ArgumentError, 'invalid value for "period", must be greater than or equal to 0.'
      end
    ...
    def period_type=(period_type)
      validator = EnumAttributeValidator.new('String', ['Month', 'Year', 'Day', 'Week'])
    ...
    def term_type=(term_type)
      validator = EnumAttributeValidator.new('String', ['TERMED', 'EVERGREEN'])
    ...
    def hash
      [period, period_type, start_date, term_type].hash
    end
