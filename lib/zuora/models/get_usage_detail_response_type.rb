...
module Zuora
  class GetUsageDetailResponseType
    # The ID of the user who imported this usage file. 
    attr_accessor :created_by

    # The date and time when you submit the request of importing the usage file.      
    attr_accessor :created_on

    # The description of the import file.   You can add some comments about the import file in this field. For example, `This import file contains 50 usage records related to four customer accounts.` 
    attr_accessor :description

    # The error message of the usage import failure.   The value of this field is the detailed error message if the usage import status is `FAILED`. Otherwise, the value of this field is `null`.  
    attr_accessor :error

    # The number of usage records with validation errors.   If a validation error occurs in any usage record, no usage records will be imported from the usage import file.  
    attr_accessor :error_count

    # The ID of the usage import. 
    attr_accessor :id

    # The number of usage records that are imported successfully.  
    attr_accessor :imported_count

    # The name of the usage import file. 
    attr_accessor :name

    # The date and time when the file processing is completed. 
    attr_accessor :process_end

    # The date and time when the file processing is started. 
    attr_accessor :process_start

    # The processing status of the usage import file. 
    attr_accessor :status

    # The total number of usage records that are validated.  
    attr_accessor :total_count

    # The ID of the user who updated the import file. 
    attr_accessor :updated_by

    # The date and time when the progress of the file processing is updated.       
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'importedCount')
        self.imported_count = attributes[:'importedCount']
      end
      ...
      if attributes.has_key?(:'processEnd')
        self.process_end = attributes[:'processEnd']
      end

      if attributes.has_key?(:'processStart')
        self.process_start = attributes[:'processStart']
      end
    ...
    def list_invalid_properties
      ...
      if !@created_by.nil? && @created_by.to_s.length > 32
        invalid_properties.push('invalid value for "created_by", the character length must be smaller than or equal to 32.')
      end
      ...
      if !@updated_by.nil? && @updated_by.to_s.length > 32
        invalid_properties.push('invalid value for "updated_by", the character length must be smaller than or equal to 32.')
      end
    ...
    def valid?
      return false if !@created_by.nil? && @created_by.to_s.length > 32
      return false if !@description.nil? && @description.to_s.length > 255
      return false if !@id.nil? && @id.to_s.length > 36
      return false if !@name.nil? && @name.to_s.length > 50
      status_validator = EnumAttributeValidator.new('String', ['PENDING', 'PROCESSING', 'COMPLETED', 'COMPLETED_WITH_ERRORS', 'FAILED'])
      return false unless status_validator.valid?(@status)
      return false if !@status.nil? && @status.to_s.length > 32
      return false if !@updated_by.nil? && @updated_by.to_s.length > 32
      true
    end
    ...
    def created_by=(created_by)
      if !created_by.nil? && created_by.to_s.length > 32
        fail ArgumentError, 'invalid value for "created_by", the character length must be smaller than or equal to 32.'
      end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ['PENDING', 'PROCESSING', 'COMPLETED', 'COMPLETED_WITH_ERRORS', 'FAILED'])
    ...
    def updated_by=(updated_by)
      if !updated_by.nil? && updated_by.to_s.length > 32
        fail ArgumentError, 'invalid value for "updated_by", the character length must be smaller than or equal to 32.'
      end
    ...
    def hash
      [created_by, created_on, description, error, error_count, id, imported_count, name, process_end, process_start, status, total_count, updated_by, updated_on].hash
    end
