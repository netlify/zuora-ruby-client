...
module Zuora
  class PutInvoiceResponseType
    ...
    # **Note:** This filed is only available if you have the Credit Balance feature enabled and the Advanced AR Settlement feature disabled.  The currency amount of the adjustment applied to the customer's credit balance. 
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('String', ['Draft', 'Posted', 'Canceled', 'Error'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ['Draft', 'Posted', 'Canceled', 'Error'])
    ...
    def hash
      [account_id, amount, auto_pay, balance, cancelled_by_id, cancelled_on, comment, created_by_id, created_date, credit_balance_adjustment_amount, currency, custom_field__c, due_date, id, invoice_date, number, posted_by_id, posted_on, status, success, target_date, tax_amount, total_tax_exempt_amount, transferred_to_accounting, updated_by_id, updated_date].hash
    end
