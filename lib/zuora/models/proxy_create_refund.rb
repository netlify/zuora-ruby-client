...
module Zuora
  ...
  class ProxyCreateRefund
    #  The ID of the account associated with this refund. This field is only required if you create a non-referenced refund. Don't specify a value for any other type of refund; Zuora associates the refund automatically with the account from the associated payment. **Character limit**: 32 **Values**: a valid account ID 
    ...
    #  Indicates how an external refund was issued to a customer. This field is only required if the `Type` field is set to ` External`. You can issue an external refund on an electronic payment. **Character limit**: 30 **Values**:  - `ACH` - `Cash` - `Check` - `CreditCard` - `Other` - `PayPal` - `WireTransfer` - `DebitCard` - `CreditCardReferenceTransaction` 
    attr_accessor :method_type

    #  The unique ID of the payment method that the customer used to make the payment. This field is only required if you create a non-referenced refund. **Character limit**: 32 **V****alues**: a valid payment method ID 
    ...
    #  The date of the refund, in `yyyy-mm-dd` format. The date of the refund cannot be before the payment date. This field is only required if the `Type` field is set to ` External`. Zuora automatically generates this field for electronic refunds. **Character limit**: 29 
    attr_accessor :refund_date

    attr_accessor :refund_invoice_payment_data
    ...
    #  Specifies whether the refund is a refund payment or a credit balance. This field is only required if you create a non-referenced refund. If you creating an non-referenced refund, then set this value to `CreditBalance`. **Character limit**: 13 **Values**:  - `Payment` - `CreditBalance` 
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'RefundInvoicePaymentData')
        self.refund_invoice_payment_data = attributes[:'RefundInvoicePaymentData']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          refund_invoice_payment_data == o.refund_invoice_payment_data &&
    ...
    def hash
      [account_id, amount, comment, gateway_option_data, gateway_state, method_type, payment_method_id, reason_code, refund_date, refund_invoice_payment_data, soft_descriptor, soft_descriptor_phone, source_type, type].hash
    end
